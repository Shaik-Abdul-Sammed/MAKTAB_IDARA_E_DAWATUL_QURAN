...
class TeacherActivityLogScreen extends StatefulWidget {
  const TeacherActivityLogScreen({super.key});

  @override
  State<TeacherActivityLogScreen> createState() => _TeacherActivityLogScreenState();
}

class _TeacherActivityLogScreenState extends State<TeacherActivityLogScreen> {
  List<TeacherAttendance> _items = [];
  ...
  void _showDetailSheet(TeacherAttendance item) {
  ...
  Widget build(BuildContext context) {
    final filteredItems = _items.where((item) => item.teacherId.toString().contains(_searchQuery)).toList();

    return Scaffold(
      backgroundColor: const Color(0xFFF9FBE7), // Cream background
      appBar: AppBar(
        title: const Text(
          'Teacher Database Audit Logs',
